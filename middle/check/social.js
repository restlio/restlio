    ...
    if(network === 'github') {
        new _r().get(network, endpoint, {}, _headers).exec((err, results) => {
            if(dot.get(results, `${network}.code`) === 200) {
                ...
                } else { // email ve username eşleşiyorsa datayı set et
                    req.__social = {email, user: username, name: body.name || username};
                    return next();
                }
            } else { // eğer code 200 dönmediyse hata at
                _helper.middle(next, _middle, 'check social network credentials');
            }
        });
        ...
        if( ! consumer ) {
            return _helper.middle(next, _middle, 'twitter consumer config not found');
        }
        ...
        request.get({url: endpoint, oauth, json:true}, (err, response, user) => {
            ...
            if( ! id ) {
                return _helper.middle(next, _middle, 'check social network credentials');
            }
        ...
        new _r().get(network, endpoint, {}, _headers).exec((err, results) => {
            ...
            if( ! mails.includes(_email) ) {
                return _helper.middle(next, _middle, 'check your email');
            }
