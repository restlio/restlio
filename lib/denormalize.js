...
class Denormalize {
    ...
    touch(data, inspector) {
        ...
        if( ! name ) return console.log('denormalize.touch inspector.Name not found');
        else if( ! denorm ) return console.log('denormalize.touch inspector.Denorm not found');
        else if( ! source ) return console.log('denormalize.touch data.source not found');
        else if( ! doc ) return console.log('denormalize.touch data.doc not found');
        ...
        if(denorm[source]) {
            ...
            if(_doc) {
                ...
                model.update(cond, updates, {multi: true}, (err, affected) => {
                    ...
                    return self._log.info(`${group}:AFFECTED`, affected);
                });
            }
        }
    ...
    size(doc, inspector) {
        ...
        if( ! name ) return console.log('denormalize.size inspector.Name not found');
        else if( ! size ) return console.log('denormalize.size inspector.Size not found');
        ...
        model.aggregate(aggr, (err, result) => {
            if(err) return self._log.error(group, err);
