...
class Inspector {
    ...
    operator(operation, _update, _save, props) {
        _update[operation] = {
            type       : 'object',
            alias      : operation,
            code       : operation,
        ...
        const clone = JSON.parse(JSON.stringify(_save));
    ...
    process(schema, parent, ptype) {
        const self = this;
        schema = schema || this._schema;

        _.each(schema, (value, key) => {
            let curr = value;
            const arr = (self.type(curr) === '[object Array]');
            const obj = (self.type(curr) === '[object Object]');
            ...
            if(arr && self._alias[key]) {
                dot.put(self._save, key, {
                    ftype: 'arrayOfObjects',
                    type: 'array',
                    optional: true,
                    settings: _.clone(self._schema[key][0].settings) || {initial: false},
                    alias: self._alias[key],
                    code: self._alias[key],
                    items: {
                        type: 'object',
                ...
                delete self._schema[key][0].settings;
             ...
             * get type name
             */
            let type = curr.type.name.toLowerCase();

            /**
             * save properties
             */
            const _save = dot.get(self._save, props || key);
            ...
            const allowHtml = curr.allow_html || dot.get(curr, 'settings.allow_html');
            if( ! allowHtml ) {
                _save.$html = true;
            }
            ...
            const _attrs = items ? _save.items : _save;
            ...
            } else if(curr.pattern && (type === 'string' || items === 'string')) {
            ...
            if(type === 'string' || type === 'array') {
                if(curr.minLength) _save.minLength = curr.minLength;
                if(curr.maxLength) _save.maxLength = curr.maxLength;
                if(curr.exactLength) _save.exactLength = curr.exactLength;
            }

            // number properties
            if(type === 'number') {
                if(curr.min) _save.min = curr.min;
                if(curr.max) _save.max = curr.max;
                if(curr.lt) _save.lt = curr.lt;
                if(curr.lte) _save.lte = curr.lte;
                if(curr.gt) _save.gt = curr.gt;
                if(curr.gte) _save.gte = curr.gte;
                if(curr.ne) _save.ne = curr.ne;
            }

            // equal
            if( ['array', 'string', 'number', 'boolean'].includes(type) ) {
                // öncelikli olarak settings.options'u dikkate alıyoruz
                // {key: value} şeklinde select box oluşturabileceğimiz için
                if(dot.get(curr, 'settings.options')) {
                    if(curr.settings.options.length) {
                        _save.eq = _.map(curr.settings.options, item => item.label && item.value ? item.value : item);
                    }
            ...
            // set rules
            if(curr.rules) _save.rules = curr.rules;
            else _save.rules = 'trim';

            if(curr.pair) _save.pair = curr.pair;
            if(curr.owner) _save.owner = curr.owner;
            if(curr.entity_acl) _save.entity_acl = curr.entity_acl; // set owner
            if(curr.belongs_to) _save.belongs_to = curr.belongs_to;
            if(curr.depends) _save.depends = curr.depends;
            if(curr.from) _save.from = curr.from;
            _save.flex_ref = curr.flex_ref ? true : false; // set flexible reference
        });
    }
    ...
    init() {
        ...
        // numbers
        const numbers = self.optypes('number', self._save);
        if(Object.keys(numbers).length) {
            self.operator('$inc', self._update, numbers);
        }

        // arrays
        const arrays = self.optypes('array', self._save);
        ...
        if(Object.keys(arrays).length) {
            ...
            const arrClone = JSON.parse(JSON.stringify(arrays));
            let arrObj;

            _.each(arrClone, (arrVal, arrKey) => {
                arrObj = {
                    type: 'object',
                    strict: false,
                    optional: true,
