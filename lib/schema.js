...
class Schema {
            ...
            case 'ValidationError':
                ...
                _.each(err.errors, (field, key) => {
                    ...
                    if(message == 'is missing and not optional')
                        slug = 'required_error';
                    else if(message.includes('must have at least key'))
                        slug = 'some_keys_error';
                    else if(message.includes('must match'))
                        slug = 'pattern_error';
                    else if(message.includes('must be longer than'))
                        slug = 'min_length_error';
                    else if(message.includes('must be shorter than'))
                        slug = 'max_length_error';
                    else if(message.includes('must have exactly'))
                        slug = 'exact_length_error';
                    else if(message.includes('must be less than'))
                    ...
                    else if(message.includes('must be equal to'))
                        slug = 'equal_error';
                    else if(message.includes('must not be equal to'))
    ...
    getFunc(obj, self, cb) {
        ...
        if( ! self._master && self.protect('get') ) // aggregation çalışabilmesi için object id'ye çevir 
        ...
        self._model.q(obj, self._config.query, (err, doc, query) => {
            ...
            else if(doc && hasOwn.call(doc, 'rows')) {
                if(self._format) {
    ...
    post(obj, cb) {
         ...
         */
        // this.header('X-Restlio-Post-Data-Masked', obj); 
        ...
        async.waterfall(a, (err, parsed, doc) => {
            ...
            if(where) {
                Item.update(where, parsed, {multi: true}, (err, raw) => {
                    ...
                    return cb ? cb(null, raw.nModified) : self._http.OK({affected: raw.n, modified: raw.nModified}, self._res);
                });
            }
