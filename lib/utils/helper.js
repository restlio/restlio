...
class Helper {
    ...
    log(level, meta) {
        if(meta.name === 'NotFound') {
            return false;
        }

        if(meta.message && this.type(meta.message) === '[object Array]') {
            const {type, errors, middleware} = meta.message;
            meta.type = type;
            meta.errors = errors;
        ...
        meta.level = level;
        meta.namestr = meta.name;
        meta.stackstr = meta.stack;
        meta.msgstr = meta.message;
        new this._app.lib.schema('system.logs').init(this._app).post(meta, err => {
            if(err) {
                console.log(err);
            }
        });
